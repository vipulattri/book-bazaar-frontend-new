...
export default function WishlistPage() {
  ...
  useEffect(() => {
    const fetchWishlist = async () => {
      ...
      try {
        ...
        const res = await fetch('https://book-bazaar-backend-new-1.onrender.com/api/wishlist', { headers: token ? { Authorization: `Bearer ${token}` } : undefined as any })
  ...
  const remove = async (bookId: string) => {
    try {
      const token = localStorage.getItem('token')
      const res = await fetch('https://book-bazaar-backend-new-1.onrender.com/api/wishlist/remove', {
        method: 'POST',
